...
class VectorStoreManager:
    ...
    def __init__(self):
        ...
        self.vectorstore_dir = Path(
            config.vectorstore.index_path).parent if has_path_separator else config.vectorstore.index_path.__class__(config.vectorstore.index_path).parent
    ...
    def _convert_distance_to_similarity(self, distance_score: float) -> float:
        ...
        # Map to accuracy score [0, 1] with realistic interpretation:
        # - Only positive similarities (>0) get good scores
        # - Negative similarities (<0) get very low scores

        if cosine_sim >= 0.95:  # Nearly identical
            accuracy = 0.95 + (cosine_sim - 0.95) * 5.0  # 95-100%
        elif cosine_sim >= 0.85:  # Very similar
            accuracy = 0.85 + (cosine_sim - 0.85) * 1.0  # 85-95%
        elif cosine_sim >= 0.70:  # Similar
            accuracy = 0.70 + (cosine_sim - 0.70) * 1.0  # 70-85%
        elif cosine_sim >= 0.50:  # Somewhat similar
            accuracy = 0.50 + (cosine_sim - 0.50) * 1.0  # 50-70%
        elif cosine_sim >= 0.0:  # Marginally similar
            accuracy = cosine_sim * 0.5  # 0-50%
        else:  # Not similar (negative cosine)
            accuracy = 0.0
    ...
    def _filter_by_similarity_threshold(self, docs_with_scores: List[Tuple], similarity_threshold: float) -> List[Tuple]:
        """
    ...
    def create_vector_store(self, documents: List[Document], chunks: List[DocumentChunk]) -> VectorStoreInfo:
        """
    ...
    @measure_execution_time
    def update_vector_store(self, new_documents: List[Document], new_chunks: List[DocumentChunk]) -> VectorStoreInfo:
        """
        ...
        try:
            ...
            for chunk in new_chunks:
                ...
                doc = next((d for d in new_documents if d.id ==
                           chunk.document_id), None)
                doc_name = doc.name if doc else "Unknown"
    ...
    def search_documents_for_openai(self, query: str, api_key: str, k: int = 5, similarity_threshold: float = 0.70, temperature: float = None, max_tokens: int = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if self.current_model_provider == "OpenAI (API)":
                llm = ChatOpenAI(
                    ...
                    temperature=temperature if temperature is not None else config.model.openai_temperature,
                    max_tokens=max_tokens if max_tokens is not None else config.model.openai_max_tokens
                )
                model_display_name = f"OpenAI {config.model.openai_model}"
            elif self.current_model_provider == "Local LLM (Qwen)":
                llm = ChatOpenAI(
                    temperature=temperature if temperature is not None else config.model.local_temperature,
                    model_name=config.model.local_model,
                    openai_api_key='ollama',
                    openai_api_base=config.model.ollama_base_url,
                    max_tokens=max_tokens if max_tokens is not None else config.model.local_max_tokens
                )
            ...
            qa_prompt_template = """You are an intelligent assistant for question-answering tasks.
Use the following pieces of retrieved context to answer the question accurately and comprehensively.

CRITICAL INSTRUCTIONS:
- If you can find ANY relevant information in the context, provide a complete answer based on it
- ONLY say you lack information if the context is COMPLETELY unrelated to the question
- Even partial information should result in a helpful answer with a note about limitations
- Extract and synthesize ALL relevant details from the provided context
...
Helpful Answer (based on context):"""
            ...
            # Step 1: Create base retriever with broader search
            base_retriever = self.vector_store.as_retriever(
                search_type="similarity",
                search_kwargs={
                    "k": k * 2,  # Get 2x more candidates for better filtering
                }
            )
            ...
            if chain_sources:
                ...
                # Step 3: Match compressed docs with original docs using chunk_id
                matched_sources = []
                for compressed_doc in chain_sources:
                    ...
                    if chunk_id and chunk_id in doc_scores_map:
                        # Found the original document's score
                        ...
                        if similarity_score >= similarity_threshold:
                            chain_sources_with_scores[compressed_doc.page_content] = (
                                distance_score, similarity_score)
                            matched_sources.append(compressed_doc)

                            self.logger.info(
                                f"  ✓ Matched & Passed: chunk_id={chunk_id}, distance={distance_score:.4f}, "
                                f"accuracy={similarity_score:.2%} >= threshold={similarity_threshold:.2%}")
                        else:
                            self.logger.info(
                                f"  ✗ Matched but Filtered: chunk_id={chunk_id}, "
                                f"accuracy={similarity_score:.2%} < threshold={similarity_threshold:.2%}")
                    else:
                        self.logger.warning(
                            f"  ⚠ Could not match compressed doc with chunk_id={chunk_id}")

                # Replace chain_sources with filtered sources
                chain_sources = matched_sources

                self.logger.info(
                    f"After similarity threshold filter ({similarity_threshold:.2%}): {len(chain_sources)} sources remain")

                # Step 4: Apply k limit (Number of Sources)
                if len(chain_sources) > k:
                    self.logger.info(
                        f"Limiting from {len(chain_sources)} to {k} sources (user setting)")
            ...
            for i, doc in enumerate(chain_sources):
                ...
                page = (doc.metadata.get('page') or
                        doc.metadata.get('chunk_index', 0) + 1 or
                        'Unknown')
            ...
            self.logger.info(
                f"Accuracy calculation: total_accuracy={total_accuracy:.4f}, sources={len(sources)}, avg_accuracy={avg_accuracy:.4f}")

            # Get the answer from response
            answer_text = response.get('answer', '') or (
                response['chat_history'][-1].content if response.get('chat_history') else 'No response generated')
            ...
            if not has_insufficient_info and len(answer_text.strip()) < 50:
                short_insufficient_phrases = [
                    "no information", "cannot answer", "don't know"]
                has_insufficient_info = any(
        ...
        if self.vector_store:
            try:
                ...
                try:
                    ...
                    index_name = Path(
                        self.index_path).name if has_path_separator else self.index_path
                    self.logger.info(
